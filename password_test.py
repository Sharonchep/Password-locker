...
class TestCredential(unittest.TestCase):
         def setUp(self):
             self.new_credential = credential("facebook", "Cynthia", "12345")
         def tearDown(self):
             Credential.credential_list = []
         def test_init(self):
                 self.assertEqual(self.new_credential.account, "facebook")
                 self.assertEqual(self.new_credential.account_username, "Cynthia")
                 self.assertEqual(self.new_credential.account_password, "12345")
         def test_save_credential(self):
                 self.new_credential.save_credential()
                 self.assertEqual(len(Credential.credential_list), 1)
         def test_save_multiple_credential(self):
                 test_credential = credential("twitter", "Ace", "0000")
